'''
    ...
    description: TCP server which sends Lepton Camera frames (temperature data) to the client
        for usage with C# script (uses <EOF> identifier to see that whole message received)
'''
...
def send_temperature_data_back(temperature_data, connection):
    serialized_data = json.dumps({'resolution': [frame_width, frame_height], 'temperatures': temperature_data.tolist()}) + "<EOF>"
